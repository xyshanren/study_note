...
author:
  - 守一
created: 2021-03-02
description: Linux使用笔记，包括一些简单的shell命令以及Linux系统配置等
tags:
  - Linux
  - shell
  - ssh
...
# 查找指定目录中 DRUG 开头的文件
find /root/download -type f -name "DRUG*"
# 查找指定目录中 DRUG 开头的文件并删除
find /root/download -type f -name "DRUG*" | xargs rm -f
# 找指定目录中 INDI 开头的文件并打包
find /root/download -type f -name "INDI*" | xargs tar -cvf INDI.tar
...
仓库文件
  .d          directory目录
  xxxd        daemon(守护进程)
  /etc/yum.repos.d/xxx.repo   仓库文件地址
...
>这里使用了阿里云源替换本地源，是直接使用阿里的仓库文件覆盖了原仓库文件。  

```shell
# 下载文件到指定文件
# -O 是指输出到指定的文件
wget -O /etc/yum.repos.d/CentOS-Base.repo http://mirrors.aliyun.com/repo/Centos-7.repo
# 也可以使用curl下载，下载时指定为自己的文件名，后面还需要进行替换
...
yum -y install nano
# 只下载不安装(需要管理员权限)
sudo yum install --downloadonly --downloaddir=~/rpms nano
# 下载已安装软件
sudo yum reinstall --downloadonly --downloaddir=~/rpms nano
# 安装本地的RPM文件
yum localinstall xxx.rmp

# 发现以下提示信息的话，是LC_ALL和LANGUAGE未设置
Failed to set locale, defaulting to C
# 在profile文件中添加以下配置，并使其生效
vim /etc/profile
export LC_ALL=C
source /etc/profile
```

>具体情况见：<https://www.cnblogs.com/clicli/p/6371118.html>
...
使用命令 `scp ~/.ssh/id_rsa.pub root@ip:~` 将公钥上传到服务器用户目录。  
...
# 制作ISO文件，-r 保留原文件，-o 输出的ISO文件名
mkisofs -r -o xxxx.iso ~/download/rmps
