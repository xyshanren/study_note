 ...
 * @LastEditors: 逍遥山人
 * @LastEditTime: 2020-04-17 14:57:29
 * @Descriptiong: 
 -->
...
- 查看库、表、字段等
  ...
  - 查看某一个库中的所有表
    ...
    SELECT
        ...
        table_rows num_rows,
        table_comment,
        concat(round(data_length / 1024 / 1024,2),'MB') as store_capacity
    FROM
...
- 查询锁表详情
  ...
  ```sql
  select TIMESTAMPDIFF(SECOND,date_add(trx_started, interval 8 hour)
        ,sysdate()) timediff
        ,sysdate()
        ,date_add(trx_started, interval 8 hour) trx_started
        ,id
        ,USER
        ,DB
        ,COMMAND
        ...
        ,trx_state
        ,trx_query
  from information_schema.processlist,information_schema.innodb_trx
